...
const ProductInfo = (props) => {
    ...
    useEffect(() => {
        const p = () => {
            axios.get("https://investment-compass-urnz.onrender.com/api/product/addwishlist", {
                params: {
                    param1: userId,
                    param2: product
                }
    ...
    const addWhishlist = () => {
        if (localStorage.getItem("user")) {
            if (!isWishList) {
                axios.post("https://investment-compass-urnz.onrender.com/api/product/addwishlist", { userId, product })
                    .then((res) => {
                        if (res.data.added) {
                            setWishList(true);
                        }
                ...
                axios.delete("https://investment-compass-urnz.onrender.com/api/product/addwishlist", {
                    data: {
                        userId: userId,
                        product: product
                    }
    ...
    const deleteCourse = () => {
        try {
            axios.delete("https://investment-compass-urnz.onrender.com/api/product/delete", {
                data: {
                    userId: userId,
                    product: product
                }
    ...
    const updateCourseAction = () => {
        ...
        axios.post("https://investment-compass-urnz.onrender.com/api/product/updateCourse", {
    ...
    const saveBuyData = async () => {
        const { data } = await axios.post("https://investment-compass-urnz.onrender.com/api/payment/save",
            {
    ...
    const initPayment = (data) => {
        const options = {
            ...
            image: `https://investment-compass-urnz.onrender.com/uploads/${d}`,
            order_id: data.id,
            handler: async (response) => {
                try {
                    const whoBought = `${JSON.parse(localStorage.getItem("user")).fname} ${JSON.parse(localStorage.getItem("user")).lname}`
                    const { data } = await axios.post("https://investment-compass-urnz.onrender.com/api/payment/verify", { response, refId, whoBought, userId });
    ...
    const handlePayment = async (req, res) => {
        try {
            const { data } = await axios.post("https://investment-compass-urnz.onrender.com/api/payment/orders", {
        ...
        <div className='productTab'>
            ...
            <div className="imageTab">
                <div className='firstImageTab'>
                    {
                        product && product.pimage && product.pimage.length ? <img style={{ borderRadius: "20px" }} src={"https://investment-compass-urnz.onrender.com/uploads/" + product.pimage[firstImgSrc]} alt="" /> : null
                    }
                </div>
                <div className='remainingImageTab'>
                    {
                        product && product.pimage && product.pimage.length > 1 ? product.pimage.map((val, index) => {
                            return (
                                <img key={index} style={{ borderRadius: "10px", cursor: "pointer", border: index === firstImgSrc ? "2px solid black" : null }} src={"https://investment-compass-urnz.onrender.com/uploads/" + val} alt="Photo" onClick={() => setFirstImgSrc(index)} />
                            )
                        }) : null
                    }
                </div>
