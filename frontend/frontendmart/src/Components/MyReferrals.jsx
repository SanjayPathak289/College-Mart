...
const MyReferrals = () => {
    ...
    useEffect(() => {
        const getR = async () => {
            ...
            const { data } = await axios.get("http://localhost:3000/referral/myreferrals", {
                params: {
    ...
    useEffect(() => {
        const getR = async () => {
            ...
            const { data } = await axios.get("http://localhost:3000/api/user/sales", {
                params: {
                    id: JSON.parse(localStorage.getItem("user"))._id
                }
            })
        ...
        <div id='myreferralDiv'>
            ...
            {referralsArr.map((product) => (
                <div className='eachReferralDiv'>
                    ...
                    <div>
                        <a href={"http://localhost:5173/referral?id=" + product.refId} style={{ textDecoration: "none" }}>{"http://localhost:5173/referral?id=" + product.refId}</a>
